...
class TermSpecification extends Properties("Term"):
  property("Variable with valid named should be created") =
    forAll(genValidVarName): (name: String) =>
      Variable(name).isRight

  property("Variables can not have non alphabetic symbols in the name") =
    forAll(genInvalidVarName): (name: String) =>
